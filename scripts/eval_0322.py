...
def main(cfg):
    ...
    def evaluate(
        ...
        # 记录扰动和轨迹误差
        gt_disturbance = trajs[("next", "info", "gt_disturbance")]
        tracking_error = trajs[("next", "stats", "tracking_error")]

        print(f"Mean disturbance: {gt_disturbance.abs().mean().item()}")
        print(f"Mean tracking error: {tracking_error.mean().item()}")
