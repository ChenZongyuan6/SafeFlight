...
def main(cfg):
    ...
    for i, traj in enumerate(traj_types):
        ...
        summary_rows.append((traj, tracking_err, ret, episode_len))

    # Log everything in one wandb step
    run.log(all_info)

    # Print comparison tables
    def print_table(title, col, idx):
        print("\n" + "=" * 50)
        print(f"  {title}")
        print(f"{'Trajectory':<15}  {col:>15}")
        print("-" * 50)
        for row in summary_rows:
            print(f"{row[0]:<15}  {row[idx]:>15.4f}")
        print("=" * 50)

    print_table("tracking_error", "tracking_error", 1)
    print_table("return",         "return",         2)
    print_table("episode_len",    "episode_len",    3)
