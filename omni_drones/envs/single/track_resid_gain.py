...
class TrackResidGain(IsaacEnv):
    """
    结构化增益补偿环境。

    与 TrackResidual 的核心区别：
    - 策略仅输出 2 维增益 [K_phi, K_theta]
    - delta_CT 由解析公式计算（不学习）
    - delta_phi_des, delta_theta_des 由解析公式计算（不学习）
    - 体速率补偿 delta_p = K_phi * delta_phi_des, delta_q = K_theta * delta_theta_des
    - delta_r = 0
    """
    ...
    def __init__(self, cfg, headless):
        ...
        self.gain_alpha = float(cfg.task.get("gain_alpha", 0.7))
        self.gain_K_max = float(cfg.task.get("gain_K_max", 5.0))
        self.gain_penalty_coef = float(cfg.task.get("gain_penalty_coef", 0.0))
        self.gain_smoothness_coef = float(cfg.task.get("gain_smoothness_coef", 0.002))
        self.delta_ct_limit = float(cfg.task.get("delta_ct_limit", 0.4))
        self.delta_rate_limit = float(cfg.task.get("delta_rate_limit", 0.5))
        self.comp_rpos_steps = int(cfg.task.get("comp_rpos_steps", 6))
        # gain_obs = delta_analytical(3) + u_base(4) + v_body(3) + w_body(3) + R_flat(9) + rpos_body(comp_rpos_steps*3) + prev_rpos0(3) + vel_error(3)
        self.gain_obs_dim = 3 + 4 + 3 + 3 + 9 + self.comp_rpos_steps * 3 + 3 + 3
        self.gain_action_dim = int(cfg.task.get("gain_action_dim", 2))

        super().__init__(cfg, headless)

        self.drone.initialize()

        self.total_mass = self.drone._view.get_body_masses().reshape(self.num_envs, -1).sum(-1, keepdim=True)
        ...
        # gain-specific buffers
        self.base_actions = torch.zeros(self.num_envs, self.num_drones, 4, device=self.device)
        self.gain_actions = torch.zeros(self.num_envs, self.num_drones, self.gain_action_dim, device=self.device)
        self.prev_gain_actions = torch.zeros(self.num_envs, self.num_drones, self.gain_action_dim, device=self.device)
        self.comp_ctbr = torch.zeros(self.num_envs, self.num_drones, 4, device=self.device)
    ...
    def _set_specs(self):
        # =========================================================
        # 1. 冻结主策略的 observation/state 维度（与 Track 一致）
        # =========================================================
        ...
        # =========================================================
        # 2. gain policy observation: 7 维
        #    [delta_phi_des(1), delta_theta_des(1), p(1), q(1), r(1), phi(1), theta(1)]
        # =========================================================
        self.observation_spec = CompositeSpec({
            "agents": {
                "observation": UnboundedContinuousTensorSpec((1, self.gain_obs_dim)),
                "state": UnboundedContinuousTensorSpec((self.gain_obs_dim,)),
            }
        }).expand(self.num_envs).to(self.device)

        # action: 2 维 [K_phi, K_theta]
        self.action_spec = CompositeSpec({
            "agents": {
                "action": UnboundedContinuousTensorSpec((1, self.gain_action_dim)),
            }
        }).expand(self.num_envs).to(self.device)
        ...
        stats_spec = CompositeSpec({
            ...
            "gain_norm_max": UnboundedContinuousTensorSpec(1),
            "gain_norm_mean": UnboundedContinuousTensorSpec(1),
            "gain_smoothness_max": UnboundedContinuousTensorSpec(1),
            "gain_smoothness_mean": UnboundedContinuousTensorSpec(1),
            "reward_tracking_delta": UnboundedContinuousTensorSpec(1),
            "delta_ct_mean": UnboundedContinuousTensorSpec(1),
            "delta_phi_des_mean": UnboundedContinuousTensorSpec(1),
            "delta_theta_des_mean": UnboundedContinuousTensorSpec(1),
            "K_phi_mean": UnboundedContinuousTensorSpec(1),
            "K_theta_mean": UnboundedContinuousTensorSpec(1),
        }).expand(self.num_envs).to(self.device)
        ...
        info_spec = CompositeSpec({
            ...
            # orchestrator 填入的解析补偿量
            "delta_ct_analytic": UnboundedContinuousTensorSpec((1,), device=self.device),
            "delta_phi_des": UnboundedContinuousTensorSpec((1,), device=self.device),
            "delta_theta_des": UnboundedContinuousTensorSpec((1,), device=self.device),
    ...
    def _reset_idx(self, env_ids: torch.Tensor):
        ...
        self.prev_gain_actions[env_ids] = 0.
        self.comp_ctbr[env_ids] = 0.
    ...
    def _pre_sim_step(self, tensordict: TensorDictBase):
        # action: [K_phi, K_theta], 经 tanh 后在 [-1,1]
        gain_raw = tensordict[("agents", "action")]  # [N, 1, 2]

        base_action = tensordict[("info", "base_action")]  # [N, 1, 4] CTBR [-1,1]

        # 从 info 读取 orchestrator 填入的解析补偿量
        delta_ct = tensordict[("info", "delta_ct_analytic")]    # [N, 1]
        delta_phi_des = tensordict[("info", "delta_phi_des")]   # [N, 1]
        delta_theta_des = tensordict[("info", "delta_theta_des")]  # [N, 1]

        # 将 gain_raw [-1,1] 映射到 [0, K_max]
        K_phi = (gain_raw[..., 0:1] + 1.0) / 2.0 * self.gain_K_max   # [N, 1, 1]
        K_theta = (gain_raw[..., 1:2] + 1.0) / 2.0 * self.gain_K_max  # [N, 1, 1]

        # 体速率补偿 (CTBR space)
        # delta_phi_des/delta_theta_des: [N, 1] -> unsqueeze to [N, 1, 1]
        delta_p = K_phi * delta_phi_des.unsqueeze(-1)    # delta_p_rate = K_phi * delta_phi_des
        delta_q = K_theta * delta_theta_des.unsqueeze(-1)  # delta_q_rate = K_theta * delta_theta_des
        delta_r = torch.zeros_like(delta_p)                # delta_r = 0

        # 裁剪 rate 补偿
        delta_p = torch.clamp(delta_p, -self.delta_rate_limit, self.delta_rate_limit)
        delta_q = torch.clamp(delta_q, -self.delta_rate_limit, self.delta_rate_limit)

        # 裁剪 delta_CT
        delta_ct_clipped = torch.clamp(delta_ct, -self.delta_ct_limit, self.delta_ct_limit)
        delta_ct_expanded = delta_ct_clipped.unsqueeze(-1)  # [N, 1, 1]

        # comp_ctbr: [delta_p, delta_q, delta_r, delta_CT]
        comp = torch.cat([delta_p, delta_q, delta_r, delta_ct_expanded], dim=-1)  # [N, 1, 4]

        # final CTBR = base + alpha * comp
        final_ctbr = base_action + self.gain_alpha * comp
        final_ctbr = torch.clamp(final_ctbr, -1.0, 1.0)

        self.base_actions = base_action.clone()
        self.gain_actions = gain_raw.clone()
        self.comp_ctbr = comp.clone()
        ...
        self.info["pred_disturbance"] = tensordict[("info", "pred_disturbance")].clone()

        self.policy_actions = final_ctbr.clone()

        # action smoothness (final CTBR)
        ...
        # gain-specific stats
        gain_norm = torch.norm(gain_raw, dim=-1)  # [N, 1]
        gain_smooth = torch.norm(gain_raw - self.prev_gain_actions, dim=-1)  # [N, 1]
        self.stats["gain_norm_mean"].add_(gain_norm)
        self.stats["gain_norm_max"].set_(torch.max(self.stats["gain_norm_max"], gain_norm))
        self.stats["gain_smoothness_mean"].add_(gain_smooth)
        self.stats["gain_smoothness_max"].set_(torch.max(self.stats["gain_smoothness_max"], gain_smooth))

        self.stats["delta_ct_mean"].add_(delta_ct.abs())
        self.stats["delta_phi_des_mean"].add_(delta_phi_des.abs())
        self.stats["delta_theta_des_mean"].add_(delta_theta_des.abs())
        self.stats["K_phi_mean"].add_(K_phi.squeeze(-1))
        self.stats["K_theta_mean"].add_(K_theta.squeeze(-1))

        # ========================================================
        # PID: CTBR -> motor commands
        # ========================================================
    ...
    def _compute_state_and_obs(self):
        ...
        prev_rpos0 = self.rpos0.clone()
        self.rpos0 = self.rpos[:, 0, :].clone()

        # ---- base_obs for frozen base policy ----
        if self.use_ab_wolrd_pos:
            base_obs_list = [
                root_state[..., :3],
        ...
        # transform rpos to body frame for comp_rpos_steps steps
        R_mat = self.R_flat.reshape(self.num_envs, 3, 3)  # [N, 3, 3]
        rpos_world = self.rpos[:, 0:self.comp_rpos_steps, :]  # [N, S, 3]
        rpos_body = torch.einsum('nij,nkj->nki', R_mat, rpos_world)  # [N, S, 3]
        prev_rpos0_body = torch.einsum('nij,nj->ni', R_mat, prev_rpos0)  # [N, 3]

        # velocity error in body frame
        target_vel_world = (self.target_pos[:, 1, :] - self.target_pos[:, 0, :]) / (5.0 * self.dt)
        current_vel_world = root_state[..., 7:10].squeeze(1)
        self.vel_error_body = torch.einsum('nij,nj->ni', R_mat, target_vel_world - current_vel_world)

        self.info["v_body"][:] = self.v_body
        self.info["w_body"][:] = self.w_body
        self.info["R_flat"][:] = self.R_flat
        self.info["rpos0"][:] = self.rpos0
        self.info["rpos_steps"][:] = rpos_body
        self.info["prev_rpos0"][:] = prev_rpos0_body
        self.info["vel_error_body"][:] = self.vel_error_body

        # ---- gain policy observation (placeholder, orchestrator will overwrite with real 46-dim obs) ----
        gain_obs = torch.zeros(self.num_envs, self.gain_obs_dim, device=self.device)  # [N, gain_obs_dim]

        gain_state = gain_obs.clone()

        self.stats["obs_range"].set_(torch.max(torch.abs(gain_obs), dim=-1, keepdim=True).values)

        if self.use_eval:
            self.sim_data.append(gain_obs[0:1].clone())
            self.sim_rpy.append(self.drone.vel_b[0, :, 3:].clone())

        return TensorDict({
            "agents": {
                "observation": gain_obs.unsqueeze(1),  # [N, 1, gain_obs_dim] - orchestrator overwrites
                "state": gain_state,                    # [N, gain_obs_dim]
            },
            "stats": self.stats,
            "info": self.info,
        }, self.batch_size)
    ...
    def _compute_reward_and_done(self):
        ...
        # gain regularization
        gain_norm_penalty = self.gain_penalty_coef * torch.norm(self.gain_actions, dim=-1)
        gain_smooth_penalty = self.gain_smoothness_coef * torch.norm(
            self.gain_actions - self.prev_gain_actions, dim=-1
        )
        ...
        reward = reward_pure + reward_tracking_delta - gain_norm_penalty - gain_smooth_penalty
        ...
        self.stats["gain_norm_mean"].div_(torch.where(done, ep_len, torch.ones_like(ep_len)))
        self.stats["gain_smoothness_mean"].div_(torch.where(done, ep_len, torch.ones_like(ep_len)))
        self.stats["reward_tracking_delta"].div_(torch.where(done, ep_len, torch.ones_like(ep_len)))
        self.stats["delta_ct_mean"].div_(torch.where(done, ep_len, torch.ones_like(ep_len)))
        self.stats["delta_phi_des_mean"].div_(torch.where(done, ep_len, torch.ones_like(ep_len)))
        self.stats["delta_theta_des_mean"].div_(torch.where(done, ep_len, torch.ones_like(ep_len)))
        self.stats["K_phi_mean"].div_(torch.where(done, ep_len, torch.ones_like(ep_len)))
        self.stats["K_theta_mean"].div_(torch.where(done, ep_len, torch.ones_like(ep_len)))

        self.stats["return"] += reward
        self.stats["return_pure"] += reward_pure
        self.stats["episode_len"][:] = self.progress_buf.unsqueeze(1)

        self.prev_gain_actions = self.gain_actions.clone()
