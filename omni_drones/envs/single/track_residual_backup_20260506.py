...
class TrackResidual(IsaacEnv):
    def __init__(self, cfg, headless):
        ...
        if self.disturbance_enable:
            ...
            # 初始化 sinsum 模式所需的 buffer（复用原有的 wind buffer）
            if self.dist_mode == "sinsum":
                if dist_cfg.get("sinsum", {}).get("intensity_range"):
    ...
    def _pre_sim_step(self, tensordict: TensorDictBase):
        ...
        if self.wind:
            ...
            wind_accel = self.wind_i * torch.sin(t * self.wind_w).sum(-1)  # [num_envs, 3]
            # get_body_masses() 返回 [num_envs, num_drones, num_links] (3D)，先 reshape 再 sum
            mass = self.drone._view.get_body_masses().reshape(self.num_envs, -1).sum(-1, keepdim=True)  # [num_envs, 1]
            wind_forces = (mass * wind_accel).view(self.num_envs, 1, 3)      # [num_envs, 1, 3]
        ...
        if self.expose_gt_disturbance:
            ...
            # 选择外部加速度源：新扰动模型 > 旧风扰动 > 无
            if self.disturbance_enable:
                ext_acc_world = self.dist_acc.unsqueeze(1)             # [N, 1, 3]
            elif self.wind:
                ext_acc_world = self.wind_accel_buf.unsqueeze(1)       # [N, 1, 3]
            else:
